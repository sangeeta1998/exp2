...
measure_native_rust() {
    ...
    for i in $(seq 1 $trials); do
        ...
        sudo ctr image rm docker.io/sangeetakakati/rust-matrix-native:$arch > /dev/null 2>&1
        script_start_time=$(date +%s%3N)
        echo "Script start time: ${script_start_time} ms"
        sudo ctr image pull docker.io/sangeetakakati/rust-matrix-native:$arch > /dev/null 2>&1
        main_start_time=$(sudo ctr run --rm docker.io/sangeetakakati/rust-matrix-native:$arch test-container 2>&1 | grep "Main function started at:" | awk '{print $5}')
...
measure_native_tinygo() {
    ...
    for i in $(seq 1 $trials); do
        # Cold start time from pull
        sudo ctr image rm docker.io/sangeetakakati/tinygo-matrix-native:$arch > /dev/null 2>&1
        script_start_time=$(date +%s%3N)
        echo "Script start time: ${script_start_time} ms"
        sudo ctr image pull docker.io/sangeetakakati/tinygo-matrix-native:$arch > /dev/null 2>&1
        main_start_time=$(sudo ctr run --rm docker.io/sangeetakakati/tinygo-matrix-native:$arch test-container 2>&1 | grep "Main function started at:" | awk '{print $5}')
        pull_time=$((main_start_time - script_start_time))
        total_pull_time=$((total_pull_time + pull_time))
