...
measure_native_cpp() {
    ...
    for i in $(seq 1 $trials); do
        # Cold start time from pull
        docker rmi sangeetakakati/cpp-matrix-native:latest > /dev/null 2>&1
        script_start_time=$(date +%s%3N)
        echo "Script start time: ${script_start_time} ms"
        main_start_time=$(docker run --platform=linux/$arch --rm sangeetakakati/cpp-matrix-native:latest 2>&1 | grep "Main function started at:" | awk '{print $5}')
        pull_time=$((main_start_time - script_start_time))
        total_pull_time=$((total_pull_time + pull_time))
        echo "C++ Trial $i (Pull, $arch): Startup Time = $pull_time ms"
    done
...
measure_wasm_cpp() {
    ...
    for arch in "${architectures[@]}"; do
        ...
        for i in $(seq 1 $trials); do
            # Cold start time from pull
            docker rmi sangeetakakati/cpp-matrix-wasm:wasm > /dev/null 2>&1
            script_start_time=$(date +%s%3N)
            echo "Script start time: ${script_start_time} ms"
            main_start_time=$(docker run --runtime=io.containerd.wasmtime.v2 --platform=wasm --rm sangeetakakati/cpp-matrix-wasm:wasm 2>&1 | grep "Main function started at:" | awk '{print $5}')
            pull_time=$((main_start_time - script_start_time))
            total_pull_time=$((total_pull_time + pull_time))
            echo "C++ Wasm Trial $i (Pull, $arch): Startup Time = $pull_time ms"
        done

        echo "Average C++ Wasm cold start time from pull ($arch): $((total_pull_time / trials)) ms"
    done
}
