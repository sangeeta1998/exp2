...
measure_main_function_start_time() {
    ...
    for (( i=1; i<=iterations; i++ )); do
        ...
        if [ -z "$runtime" ]; then
            ...
            main_start_time=$(docker run --platform=linux/$platform --rm $image 2>&1 | grep "Main function started at:" | awk '{print $5}')
        else
            # For Wasm containers with specific runtime
            main_start_time=$(docker run --runtime=$runtime --platform=$platform --rm $image 2>&1 | grep "Main function started at:" | awk '{print $5}')
        fi
        end_time=$(date +%s%N)
...
measure_main_function_start_time "$rust_wasm_image" "io.containerd.wasmtime.v1" "wasm" true $iterations

measure_main_function_start_time "$tinygo_native_image" "" "$arch" true $iterations
measure_main_function_start_time "$tinygo_wasm_image" "io.containerd.wasmtime.v1" "wasm" true $iterations

measure_main_function_start_time "$cpp_native_image" "" "$arch" true $iterations
measure_main_function_start_time "$cpp_wasm_image" "io.containerd.wasmtime.v1" "wasm" true $iterations
...
if [[ "$(docker images -q $rust_wasm_image 2> /dev/null)" != "" ]]; then
    ...
    measure_main_function_start_time "$rust_wasm_image" "io.containerd.wasmtime.v1" "wasm" false $iterations
else
...
if [[ "$(docker images -q $tinygo_native_image 2> /dev/null)" != "" ]]; then
    ...
    measure_main_function_start_time "$tinygo_native_image" "" "$arch" false $iterations
else
...
if [[ "$(docker images -q $tinygo_wasm_image 2> /dev/null)" != "" ]]; then
    ...
    measure_main_function_start_time "$tinygo_wasm_image" "io.containerd.wasmtime.v1" "wasm" false $iterations
else
...
if [[ "$(docker images -q $cpp_native_image 2> /dev/null)" != "" ]]; then
    ...
    measure_main_function_start_time "$cpp_native_image" "" "$arch" false $iterations
else
...
if [[ "$(docker images -q $cpp_wasm_image 2> /dev/null)" != "" ]]; then
    ...
    measure_main_function_start_time "$cpp_wasm_image" "io.containerd.wasmtime.v1" "wasm" false $iterations
else
